...
def load_file_scan(animal="Patient", class_name="ictal"):
...
def extract_and_concat_features(signal_data_file):
    ...
    return X_train_sample, train_label
...
def main():
    ...
    for mat_file_idx, mat_file in enumerate(mat_files):
        X_train, Y_train = extract_and_concat_features(mat_file)
    ...
    df.to_csv("Revised_train_%s_%s_data_v1.csv" % (animal_name, class_name))
