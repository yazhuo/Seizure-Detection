...
# DATA_SRC_PATH = "../../../data/"
DATA_SRC_PATH = "../../DataSet/" # TODO: change
LABEL_CSV_FILE = "SzDetectionAnswerKey.csv"
...
def load_file_scan(animal="Dog", class_name="ictal"):
    # regex_files = os.path.join(DATA_SRC_PATH, "%s_*" % animal, "%s_*_%s_segment_*.mat" % (animal, class_name))
    regex_files = os.path.join(DATA_SRC_PATH, "%s_*" % animal, "%s_*_test_segment_*.mat" % animal)
...
def extract_and_concat_features(signal_data_file):
    ...
    return X_train_sample, data.shape[0]
...
def main(patient_idx, class_name):
    ...
    mat_files = load_file_scan(animal=animal_name, class_name=class_name)
    ...
    for mat_file_idx, mat_file in enumerate(mat_files):
        mat_rel_file = mat_file.split("/")[-1]
        # print(mat_rel_file) # TODO: remove

        # ====================================> Look-up test class labels
        matched_files = label_df[label_df["clip"] == mat_rel_file]

        if len(matched_files) != 1:
            print("Error in matched files !!!")
            list_seizure_labels.append(np.nan)
            list_early_labels.append(np.nan)
        else:
            matched_row = matched_files.iloc[0]
            list_seizure_labels.append(matched_row.iloc[1])
            list_early_labels.append(matched_row.iloc[2])

        X_train, n_channels = extract_and_concat_features(mat_file)
    ...
    seizure_labels = np.asarray(list_seizure_labels)
    early_labels = np.asarray(list_early_labels)
    ...
    df.insert(1, 'seizure labels', seizure_labels)
    df.insert(2, 'early labels', list_early_labels)
    # df.to_csv("Revised_test_%s_%s_data_v1.csv" % (animal_name, class_name))
    df.to_csv("Revised_test_%s_%d_%s_data_v1.csv" % (animal_name, patient_idx, class_name))
